 ...
 */
class StateRegexAuthManager extends Component implements ManagerInterface
{
    public $regex = null;

    public static function getRegexForUser($id)
    {
    ...
    {
//        return ".*";
//        Yii::log("Allowed access objects: q111", CLogger::LEVEL_TRACE);
        if(!Yii::$app->user->isGuest)
        {
            if ($this->regex===null)
            {
                $regexes = Yii::$app->session->get('auth-regexes');
                $id = Yii::$app->user->getId();

                if (!is_array($regexes) || YII_DEBUG)
                {
                    $regexes = Yii::$app->db->createCommand("SELECT DISTINCT(object) FROM(
                    SELECT * FROM AccessUser AU WHERE AU.user=".$id."
                    UNION
                    SELECT * FROM AccessGroup AG WHERE AG.group IN (SELECT UGU.group FROM UserGroupUser UGU WHERE UGU.user= ".$id.")
                  )  T")->queryColumn();
                    Yii::$app->session->set('auth-regexes', $regexes);
                }
//            Yii::log("Allowed access objects: ".implode(",", $regexes));
...
//            throw new NotFoundHttpException('The requested page does not exist.');
    ...
    {
//        die;
//        Yii::log("$itemName".implode(",", $regexes));
        ...
        if (preg_match("/".$this->getRegex()."/", $permissionName))
            return true;
        else
            return false;
    }
